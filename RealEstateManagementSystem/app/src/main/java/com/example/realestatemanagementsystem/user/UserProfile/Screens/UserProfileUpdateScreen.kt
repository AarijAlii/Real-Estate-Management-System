...
fun UserProfileUpdateScreen(
    ...
    // Fetch user profile in LaunchedEffect
        ...
        if (userProfile != null) {
            ...
            Column(verticalArrangement = Arrangement.Center, modifier = Modifier.padding(16.dp)) {
                ...
                Column(verticalArrangement = spacedBy(16.dp)) {
                    ...
                    TextField(value = contact, onValueChange = { contact = it }, label = { Text("Contact") }, modifier = Modifier.fillMaxWidth())
                    TextField(value = city, onValueChange = { city = it }, label = { Text("City") }, modifier = Modifier.fillMaxWidth())
                    TextField(value = region, onValueChange = { region = it }, label = { Text("Region") }, modifier = Modifier.fillMaxWidth())
                    TextField(value = postalCode, onValueChange = { postalCode = it }, label = { Text("Postal Code") }, modifier = Modifier.fillMaxWidth())

                    Button(
                        onClick = {
                            // Update user profile in Room and Firestore
                            profileViewModel.updateUserProfileInFirestoreAndRoom(
                                email = email,
                                firstName = firstName,
                                lastName = lastName,
                                contact = contact,
